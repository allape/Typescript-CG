...
const template: Template = {
  ...
  '.component.ts': (config, camelName, CamelName, UNDER_LINE, under_line, fieldMaxLength) => {
    ...
    rows.push(`})`);
    rows.push(`export class ${CamelName}Component extends BaseComponent<${CamelName}> implements OnInit {`);
    rows.push(``);
    ...
    rows.push(`  });`);
    rows.push(``);
    rows.push(`  beforeLoad(page: number, size: number, data: any | ${CamelName}): any {}`);
    rows.push(``);
    rows.push(`  afterLoad(res: PageRes<${CamelName}>): any {`);
    if (dateFields.length > 0) {
      rows.push(`    DateUtils.listDateStringFormat(`);
      rows.push(`      res.list, DateUtils.DEFAULT_DATETIME_FORMAT, '-', null,`);
      rows.push(`      ...this.DATETIME_FIELDS`);
      rows.push(`    );`);
    }
    rows.push(`  }`);
    rows.push(``);
    ...
    rows.push(`   * 新增修改表单`);
    rows.push(`   */`);
    rows.push(`  public saveForm: FormGroup = this.fb.group({`);
    // 新增修改表单字段
    const saveForm = [];
    for (const field of config.fields) {
    ...
    rows.push(`   * 表单默认填充数据`);
    rows.push(`   */`);
    rows.push(`  public readonly saveFormDefaultValues = {`);
    for (const field of saveForm) {
    ...
    rows.push(`    this.saveForm.reset(this.saveFormDefaultValues);`);
    rows.push(`    this.saveFormReadonly = readonly;`);
    rows.push(``);
    rows.push(`    if (item) {`);
    rows.push(`      this.saveForm.patchValue(item);`);
    rows.push(`    }`);
    rows.push(``);
    ...
    rows.push(`      nzFooter: this.saveFormReadonly ? null : undefined,`);
    rows.push(`      nzOnOk: () => {`);
    rows.push(`        return this.doSaveModal();`);
    rows.push(`      },`);
    rows.push(`    });`);
  ...
  '.component.html': (config, camelName, CamelName, UNDER_LINE) => {
    ...
    rows.push(`  <form nz-form [formGroup]="saveForm" nzLayout="vertical">`);
    rows.push(`    <div nz-row nzGutter="10">`);
    for (const field of config.fields) {
      if (field.saveModalConfig) {
